    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            await SupabaseService.addPayment({
                ...
                amount: createdUser.planValue!,
                status: 'PENDING',
                dueDate: new Date().toISOString().split('T')[0],
                description: `Primeira mensalidade - ${selectedPlan.title}`
            });
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {plansByType[type].map(plan => (
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-500 text-xs mt-1">{plan.frequency}</p>
                                        <p className={`font-black text-lg mt-2 ${selectedPlanId === plan.id ? 'text-brand-600' : 'text-gray-800'}`}>R$ {plan.price.toFixed(2)}</p>
                                    </button>
                                ))}
                            </div>
                        </div>
            ...
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl w-full max-w-md border border-gray-200 text-center animate-fade-in relative overflow-hidden">
                ...
                {step === 'CODE_INPUT' && (
                    <form onSubmit={handleCodeValidation} className="space-y-4">
                        ...
                        <button type="submit" className="w-full bg-brand-600 text-white font-black py-5 rounded-2xl uppercase tracking-widest shadow-lg shadow-brand-600/20" disabled={isLoading}>{isLoading ? <Loader2 className="animate-spin mx-auto"/> : 'Validar'}</button>
                    </form>
                )}
                {step === 'FORM_INPUT' && (
                    <form onSubmit={(e) => { e.preventDefault(); setStep('PLAN_SELECTION'); }} className="space-y-4">
                        ...
                        <div className="relative"><input type={showPassword ? "text" : "password"} required className="w-full bg-gray-100 border-gray-300 rounded-2xl p-5 text-gray-900 focus:border-brand-500 outline-none pr-12" placeholder="Crie sua Senha" value={password} onChange={e => setPassword(e.target.value)} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-500">{showPassword ? <EyeOff/> : <Eye/>}</button></div>
                        ...
                        <button type="submit" className="w-full bg-brand-600 text-white font-black py-5 rounded-2xl uppercase tracking-widest shadow-lg shadow-brand-600/20">Avançar</button>
                    </form>
                )}
                {step === 'PLAN_SELECTION' && (
                    <div className="text-left">
                        <div className="mb-6"><h3 className="text-xl font-black text-gray-800 flex items-center gap-2"><HandCoins className="text-brand-500"/> Selecione seu plano</h3><p className="text-sm text-gray-500">Este será seu plano inicial no Studio.</p></div>
                        {isLoading ? <Loader2 className="animate-spin mx-auto text-brand-500" /> : renderPlanCards()}
                        <div className="mt-6"><button onClick={handleRegister} disabled={!selectedPlanId || isLoading} className="w-full bg-brand-600 text-white font-black py-5 rounded-2xl uppercase tracking-widest shadow-lg shadow-brand-600/20 disabled:opacity-50">{isLoading ? <Loader2 className="animate-spin mx-auto"/> : 'Finalizar Cadastro'}</button></div>
                    </div>
                )}
