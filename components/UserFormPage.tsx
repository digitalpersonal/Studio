    ...
    <div className="max-w-4xl mx-auto bg-dark-950 p-8 rounded-[2.5rem] border border-dark-800 shadow-2xl space-y-6 animate-fade-in mb-20">
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input className={`w-full bg-dark-900 border ${!formData.name?.trim() ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none transition-all`} value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-[10px] font-bold uppercase mb-1"><RequiredLabel text="E-mail"/></label>
                    <input type="email" className={`w-full bg-dark-900 border ${!formData.email?.trim() ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none transition-all`} value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} />
                  </div>
                  <div>
                  ...
                  <div className="relative group">
                      ...
                      <div className="relative">
                          <input 
                            ...
                            className={`w-full bg-dark-900 border ${!formData.password?.trim() ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none pr-12 font-mono transition-all`} 
                  ...
                  <div className="sm:col-span-2">
                      ...
                      {isAdmin ? (
                        <select
                            required
                            className="w-full bg-dark-900 border border-dark-700 rounded-xl p-3 text-white font-bold focus:border-brand-500 outline-none text-sm"
                  ...
                  <div className="sm:col-span-2 border-t border-dark-800 pt-4 mt-2">
                    ...
                    <input 
                      ...
                      className={`w-full bg-dark-900 border ${birthDateMasked.length !== 10 ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none transition-all`} 
                  ...
                  {(formData.role === UserRole.STUDENT || !isAdmin) && (
                    <>
                      <div>
                        ...
                        <input className={`w-full bg-dark-900 border ${!formData.cpf?.trim() ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none transition-all`} placeholder="000.000.000-00" value={formData.cpf || ''} onChange={e => setFormData({...formData, cpf: e.target.value})} />
                      </div>
                      <div>
                        <label className="block text-slate-500 text-[10px] font-bold uppercase mb-1"><RequiredLabel text="RG"/></label>
                        <input className={`w-full bg-dark-900 border ${!formData.rg?.trim() ? 'border-dark-700 focus:border-brand-500' : 'border-emerald-500/30'} rounded-xl p-3 text-white font-bold outline-none transition-all`} placeholder="MG-00.000.000" value={formData.rg || ''} onChange={e => setFormData({...formData, rg: e.target.value})} />
                      </div>
                    </>
                  )}
              </div>
      ...
      <div className={`p-6 border-t border-dark-800 flex flex-col sm:flex-row gap-4 bg-dark-950 rounded-b-2xl ${isCompletingProfile ? '-mx-8 -mb-8' : ''}`}>
        ...
        <button 
          form="user-form"
          type="submit" 
          className={`flex-1 py-5 ${totalMissing > 0 ? 'bg-dark-800 text-slate-600' : 'bg-brand-600 text-white shadow-brand-600/30 hover:bg-brand-500'} font-black rounded-2xl uppercase text-[10px] tracking-widest shadow-2xl transition-all flex items-center justify-center gap-2`}
        >
          {totalMissing > 0 ? <AlertTriangle size={18} /> : <Save size={18} />}
          {isCompletingProfile ? 'Confirmar e Finalizar' : (editingUser ? 'Salvar Alterações' : 'Cadastrar Usuário')}
        </button>
      </div>
    </div>
  );
  ...
  <div>
    ...
    {isChecked && (
      <div className="mt-3 animate-fade-in">
        <textarea required={required} placeholder={placeholder} className="w-full h-24 bg-dark-900 border border-dark-700 rounded-xl p-4 text-white font-bold text-sm focus:border-brand-500 outline-none resize-none" value={value} onChange={(e) => onTextChange(e.target.value)} />
      </div>
    )}
  </div>
);
