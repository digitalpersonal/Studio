...
export const ReportsPage: React.FC<ReportsPageProps> = ({ currentUser, onNavigate, addToast }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const history = await SupabaseService.getFinancialReport(currentYear);
        setFinancialHistory(history);
      } catch (e) { addToast(`Erro ao carregar dados financeiros.`, "error"); } finally { setLoading(false); }
    };
    fetchData();
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20 printable-area">
      ...
      <div className="bg-dark-950 p-8 rounded-[2.5rem] border border-dark-800 shadow-2xl h-80 flex items-center justify-center">
          <p className="text-slate-600 font-black uppercase text-xs">Visualização Gráfica de Gestão</p>
      </div>
    </div>
  );
};
